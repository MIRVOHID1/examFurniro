...
const Index: React.FC = () => {
    ...
    return (
        <div className="container mx-auto">
            <div className="grid gap-10 space-between grid-cols-4 mt-[60px]">
                {loading ? (
                    Array.from({ length: 12 }).map((_, index) => <Skeleton key={index} />)
                ) : (
                    products.map((product, index) => (
                        <Link href={`/shop/${product._id}`} key={product._id || index} className="relative w-[285px] h-[480px] bg-[#F4F5F7] shadow-md overflow-hidden group">
                            <div>
                            ...
                            <div className="absolute inset-0 bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 flex flex-col justify-center items-center transition-opacity duration-300 ease-in-out">
                                <button onClick={(e) => addToCart(e, product)} className="bg-white text-black px-4 py-2 mb-2">
